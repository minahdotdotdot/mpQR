...
\begin{equation}
d(m,u^h, u^l) := \lceil (u^l+mu^h)/u^l\rceil,\label{eqn:d}
\end{equation} 
so that if $\|\hat{\bb{x}}-\bb{x}\|_2 \leq \gamma_m^h$, then $\|\text{\tt castdown}(\hat{\bb{x}})-\bb{x}\|_2 \leq \gamma_{d(m,u^h, u^l)}^l.$
This is a looser bound but it allows us to compare the errors to the uniform, low precision implementation of forming $\hat{\bb{x}}$ easily.
...
\begin{align*}
	...
	\|\hat{\bb{Q}}_{TSQR,L}\|_F&\leq u^l+n(L\tilde{\gamma}_{2n}^h+\tilde{\gamma}_{m2^{-L}}^h) \leq \tilde{\gamma}_{d(n(L2n+m2^{-L}),u^h,u^l)}^l.
\end{align*}
