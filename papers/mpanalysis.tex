...
\begin{algorithm2e}
	...
	\For{$k=1:N-1$}{
		...
		$[\bb{C}_{k+1}\cdots\bb{C}_{N}]$ -= $\bb{V}_{k} \left(\bb{W}_{k}^{\top}[\bb{C}_{k+1}\cdots\bb{C}_{N}]\right) $ \tcc*{returned in high precision}
	}
	\tcp{Now build $\bb{Q}$ using level-3 BLAS operations.} 
	$\bb{Q}\gets \bb{I}$\tcc*{$\bb{I}_m$ if full QR, and $\bb{I}_{m\times n}$ if thin QR.}
	\For{$k=N:-1:2$}{
		\tcp{All but the last WY update are returned in high precision.}
		$\bb{Q}[(k-1)r+1:m,(k-1)r+1:n]$-= $\bb{W}_i \left(\bb{V}_k^{\top}\bb{Q}[(k-1)r+1:m,(k-1)r+1:n]\right)$
	}
...
Since $\hat{\bb{X}}_{k'}$'s are computed with \cref{algo:buildWY} then cast down, we can represent the error via
\begin{align}
	\hat{\bb{X}}_{k'} &= \text{\tt castdown}(\hat{\bb{P}}_{k'}^{(1)}\cdots\hat{\bb{P}}_{k'}^{(r)}) \\
...
\end{align}
for $k'=1:N$, and the bound in \cref{eqn:deltX} was used and \cref{lem:3.7} was invoked.
Then the columnwise error is 
\begin{align}
	\|\Delta \bb{q}_j \|_2 &\leq -1 + (1+u^{(l)})\prod_{k'=1}^k (1+u^{(l)})(1+r\tilde{\gamma}_{m-(k'-1)r})\\ 
	&\leq \gamma_{k+1}^{(l)} + kr\tilde{\gamma}_m^{(h)} + kr\gamma_{k+1}^{(l)}\tilde{\gamma}_m^{(h)}, \label{eqn:mpBQRcol}
\end{align} 
where $\Delta \bb{q}_j = (\bb{I}^{(l)}\hat{\bb{X}}_1\cdots\hat{\bb{X}}_k - \bb{X}_1\cdots\bb{X}_k )\hat{e}_j.$
Summing over the columns to find a matrix norm error bound yields
\begin{equation}
	\|\hat{\bb{Q}}_{mpBQR}-\bb{Q}\|_F \leq n^{1/2} \tilde{\gamma}_{N+1}^{(l)} + n^{(3/2)}\tilde{\gamma}_m^{(h)},
\end{equation}
where the summation of the third term in \cref{eqn:mpBQRcol} is swept under the tilde notation in $n^{1/2} \tilde{\gamma}_{N+1}^{(l)}$.
This bound shows that \cref{algo:mpBQR} only adds $n^{1/2}\gamma_{N+1}^{(l)}$ order errors to the bounds in \cref{eqn:BQRmat}.
Whether this loss in accuracy by $n^{1/2}\gamma_{N+1}^{(l)}$ is worth the speed-ups is an open question that can be tackled in future research.
We expect that the number of blocks $N$, the dimension of the input matrix $m,n$, and hardware specificities will be contributing factors. 
...
%\leq \tilde{\gamma}_{d(n^{3/2}(2Ln+m2^{-L}), u^{(h)}, Ln^{1/2}, u^{(l)})}^{(l)}
\end{align}
