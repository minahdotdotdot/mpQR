...
\begin{algorithm2e}
	...
	\For{$i=1:N$}{
		...
		$\bb{W_i},\bb{Y_i} \gets $ {\tt buildWY}$(\bb{V_i},\bm{\beta}_i)$ \tcc*{\Cref{algo:buildWY}}
		\If {$i< N$}{
			$[\bb{C_{i+1}}\cdots\bb{C_N}]$ -= $\bb{Y_i} \left(\bb{W_i}^{\top}[\bb{C_{i+1}}\cdots\bb{C_N}]\right) $ \tcc*{update the rest: BLAS-3}
		}
	}
...
\end{equation*}
We use $rN \approx n$ to simplify the bound via $$ (N-1)(m+2r-rN/2) \approx N(m+2r-n/2) \approx N(m-n/2) +2n \leq mN+2n$$
