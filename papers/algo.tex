...
Let us represent the $j^{th}$ Householder constant and vector of the $k^{th}$ block computed with FLOPs as with $\hat{\beta}_k^{(j)}$ and $\hat{\bb{v_k}}^{(j)}$ and the $j'^{th}$ update to the WY representation as $$\bb{Q_k}^{(j')} = \bb{I} - \hat{\bb{W_k}}^{(j')}\hat{\bb{Y_k}}^{(j')\top}.$$ %we write $\bb{V}_k[:,1:j]=:\bb{Y}_k^{(j)}$ to make a clear connection to \cref{lem:WY}, and also denote
...
\begin{align*}
	...
	\fl(\bb{b_j} - \fl(\hat{\bb{W_k}}\fl(\hat{\bb{Y_k}}^{\top}\bb{b}))) & =(1+\dd)(\bb{b_j} - (\hat{\bb{W_k}} + \tilde{\Delta} \bb{W_k}) (\hat{\bb{Y_k}} + \tilde{\Delta} \bb{Y_k})^{\top}\bb{b_j}),
\end{align*}
...
If we let $|\tilde{\Delta} \bb{W_k}| \leq \tilde{\gamma}_{W}|\hat{\bb{W_k}}|$ and $|\tilde{\Delta} \bb{Y_k}| \leq \tilde{\gamma}_{Y}|\hat{\bb{Y_k}}|$, then we have 
\begin{equation*}
	|\fl(\hat{\bb{Q_k}}\bb{b_j}) - \bb{Qb_j}| \leq \tilde{\gamma}_{W} \left(|\bb{b_j}|+|\hat{\bb{W_k}}||\hat{\bb{Y_k}}|^{\top}|\bb{b_j}|\right),
\end{equation*}
where I have used that $\bb{W_k}$ accrues additional rounding errors compared to $\bb{Y_k}$.
Note that we implicitly covered the same step of applying an WY update in the construction of $\bb{z_k}^{(j)}$, but used \cref{lem:19.3} instead. 
Now, we leave this error in a general form to allow for different mixed-precision settings in \cref{sec:mpanalysis}.
As a result of introducing generality, we conclude with a bound formed with little rigor: $$\fl(\hat{\bb{Q_k}}\bb{b_j}) = (\bb{Q_k} + \bb{\Delta Q_k})\bb{b_j}, \;\; |\bb{\Delta Q_k}| \lessapprox \tilde{\gamma}_{W}.$$
...
\|\bb{\Delta Q'}_{BQR}\| & \leq r\sum_{k=N}^1 \tilde{\gamma}_{\bb{W_k}} \label{eqn:BQR-mp}\\
&\leq r\sum_{k=N}^1 \tilde{\gamma}_{m-(k-1)r} \leq rN\tilde{\gamma}_m \equiv n\tilde{\gamma}_m \label{eqn:BQR}.
\end{align}
