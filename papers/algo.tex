...
\subsection{Householder QR (HQR)}
The HQR algorithm uses Householder transformations to zero out elements below the diagonal of a matrix. 
...
\begin{algorithm2e}
	...
	\label{algo:hhQR}
\end{algorithm2e}
\subsection{Block HQR with partitioned columns}
\subsection{Block HQR with partitioned rows : Tall-and-Skinny QR (TSQR)}