...
%DIF DEL revision2.tex   Mon Oct 12 09:06:29 2020
%DIF ADD revision3.tex   Tue Jan  5 10:25:13 2021
%Packages
...
However, many applications exist that would benefit from the use of low precision arithmetic and storage that are less sensitive to floating-point round-off error, such as \DIFaddbegin \DIFadd{training dense neural networks \mbox{%DIFAUXCMD
\cite{micikevicius2018mixed} }\hspace{0pt}%DIFAUXCMD
or }\DIFaddend clustering or ranking graph algorithms \cite{vonLuxburg2007}\DIFdelbegin \DIFdel{or training dense neural networks \mbox{%DIFAUXCMD
\cite{micikevicius2018mixed}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend .
As a \DIFdelbegin \DIFdel{first }\DIFdelend step towards that goal, we investigate the use of mixed precision arithmetic for the QR factorization, a widely used linear algebra routine.
\par
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Many }\DIFaddend existing rounding error analyses of linear algebra routines are built within what we call a \emph{uniform precision} setting, which is the assumption that all arithmetic operations and storage are performed via the same precision.
...
We use the first definition of backward error for the remainder of this paper\DIFaddbegin \DIFadd{, which for the QR factorization is: $\|\bb{A}-\hat{\bb{Q}}\hat{\bb{R}}\|$}\DIFaddend .
...
Thus, when two fp16 numbers are multiplied and stored in fp32, there is no roundoff error, and the summation and the \DIFdelbegin \DIFdel{final }\DIFdelend cast down operations are the only sources of rounding error in this inner product scheme \DIFaddbegin \DIFadd{if no underflow or overflow occurs at the final cast down step}\DIFaddend .
\begin{assump}
	\label{assump:mp}
	Let $l$ and $h$ each denote low and high precision types with unit round-off values $u^{(l)}$ and $u^{(h)}$, where $1 \gg u^{(l)} \gg u^{(h)} >0$ \DIFaddbegin \DIFadd{and $u^{(h)} \lesssim (u^{(l)})^2$}\DIFaddend .
...
\begin{equation}
\label{eqn:dperr_2}
\hat{s}_m = (\bb{x}[1]\bb{y}[1]+\bb{x}[2]\bb{y}[2])\prod_{k=1}^{m-1}(1+\dd_{k}^{(h)}) + \sum_{i=3}\DIFdelbegin \DIFdel{^n }\DIFdelend \DIFaddbegin \DIFadd{^m }\DIFaddend \bb{x}[i]\bb{y}[i]\left(\prod_{k=i-1}^{m-1}(1+\dd_{k}^{(h)})\right).
\end{equation}
...
Comparing this \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend the high precision inner product shows that the error bound has doubled from $mu^{(h)}$ to $2mu^{(h)}$.
...
%DIF <  but a factor of 2 or greater reduction in speed when compared to the higher precision.\par
%DIF >  but a factor of 2 or greater reduction in speed when compared with the higher precision.\par
...
\begin{lemma}\label{lem:mp}
	...
	\end{align}
\end{lemma}
Note that \cref{eqn:mpgamma} drops the term $\tilde{\gamma}_{k_l}^{(l)}\tilde{\gamma}_{k_h}^{(h)}$ since both $\tilde{\gamma}_{k_l}^{(l)}$ and $\tilde{\gamma}_{k_h}^{(h)}$ are larger than their product and this product can be swept \DIFdelbegin \DIFdel{under }\DIFdelend \DIFaddbegin \DIFadd{into }\DIFaddend the small integer $c > 0$ \DIFdelbegin \DIFdel{assumption }\DIFdelend implicitly included in the tilde notation.
Using these two mixed precision settings \DIFaddbegin \DIFadd{(TC bFMA and \mbox{%DIFAUXCMD
\cref{assump:mp}}\hspace{0pt}%DIFAUXCMD
) }\DIFaddend in HQR algorithms results in casting down to the low precision at different parts of the algorithms.
...
We have demonstrated a need for rounding error analysis that is accurate for mixed precision procedures and analyzed the inner product in an ad hoc mixed precision \DIFdelbegin \DIFdel{inner product }\DIFdelend \DIFaddbegin \DIFadd{setting }\DIFaddend that mimics the TensorCore bFMA.
We will use this to analyze various \DIFdelbegin \DIFdel{Householder (HH ) }\DIFdelend \DIFaddbegin \DIFadd{HH }\DIFaddend QR factorization algorithms.
...
\end{equation}
and $|\Delta {\bf v}[1]| = 0$.
Since $1+2(m+2) = \cO(m)$, we have swept that minor difference \DIFdelbegin \DIFdel{under our use of the }\DIFdelend \DIFaddbegin \DIFadd{into the constant defined in the }\DIFaddend $\tilde{\gamma}$ \DIFdelbegin \DIFdel{notation defined in \mbox{%DIFAUXCMD
\cref{lem:gamma}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{notation}\DIFaddend .
...
\DIFdelbegin \DIFdel{At }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend iteration $i$, we replace $\bb{x}$ with $\bb{A}[i:m,i]\in\R^{m-i+1}$ and the $i^{th}$ HH constant and vector ($\hat{\beta}_i$,$\bb{v}_i$) both have errors bounded by $\tilde{\gamma}_{m-i+1}$.
\paragraph{Applying a Single HH Transformation}
Now we consider lines 4-6 of \cref{algo:hhQR}.
\DIFdelbegin \DIFdel{At }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend iteration $i$, we set $\bb{A}[i+1:m,:]$ to zero and replace $\bb{A}[i,i]$ with $\sigma$ computed from \cref{algo:hh_v2}.
...
\begin{lemma}\label{lem:3.7}
	If $\bb{X}_{j} + \Delta \bb{X}_{j} \in\R^{m\times m}$ satisfies $\|\Delta \bb{X}_{j}\|_F\leq \tau_j \|\bb{X}_{j}\|_2$ for all $j=1,\cdots,r$, then $$\left|\left|\prod_{j=1}^r (\bb{X}_{j} + \Delta \bb{X}_{j})- \prod_{j=1}^r\bb{X}_{j} \right|\right|_F\leq\left(-1+\prod_{j=1}^r(1+\tau_j)\right)\prod_{j=1}\DIFdelbegin \DIFdel{^n}\DIFdelend \DIFaddbegin \DIFadd{^r}\DIFaddend \|\bb{X}_{j}\|_2.$$
\end{lemma}
...
\begin{proof}
	...
	\[
		\|\Delta\bb{Q}\|_F = \left|\left|\prod_{j=1}^r (\bb{P}_{j} + \Delta \bb{P}_{j})- \prod_{j=1}^r\bb{P}_{j} \right|\right|_F\leq\left(-1+\prod_{j-1}^r(1+\tilde{\gamma}_{m-j+1})^r\right)\prod_{j=1}\DIFdelbegin \DIFdel{^n}\DIFdelend \DIFaddbegin \DIFadd{^r}\DIFaddend \|\bb{P}_{j}\|_2 \leq -1+(1+\tilde{\gamma}_m)^r,\]
	since $\bb{P}_{j}$'s are orthogonal and have \DIFaddbegin \DIFadd{unit }\DIFaddend 2-norm \DIFdelbegin \DIFdel{, 1, }\DIFdelend and $m-j+1 \leq m$.
...
\begin{algorithm2e}
	...
	\Return $\bb{W}$
	\caption{$\bb{W},\bb{Y}\gets {\tt buildWY}(V, \bm{\beta})$: Given a set of \DIFdelbegin \DIFdel{householder }\DIFdelend \DIFaddbegin \DIFadd{Householder }\DIFaddend vectors $\{\bb{V}[:,i]\}_{i=1}^r$ and their corresponding constants $\{\bm{\beta}_i\}_{i=1}^r$, form the final $\bb{W}$ and $\bb{Y}$ factors of the WY representation of $\bb{P}_1\cdots \bb{P}_r$, where $\bb{P}_i := \bb{I}_m - \bm{\beta}_i\bb{v}_i\bb{v}_i^{\top}$}
	\label{algo:buildWY}
\end{algorithm2e}
...
For example, the $k^{th}$ HH transformation finds an $m-k+1$ sized HH transformation that zeroes out column $k$ below the diagonal and then applies it to the $(m-k+1)$-by-$(n-k)$ partition of the matrix \DIFdelbegin \DIFdel{, }\DIFdelend $\bb{A}[k:m,k+1:n]$.
...
BQR performs approximately $1-\cO(1/N)$ fraction of its FLOPs in Level-3 BLAS operations (see section 5.2.3 of \cite{golub2013matrix}), and can reap the benefits from the accelerated \DIFdelbegin \DIFdel{block FMA }\DIFdelend \DIFaddbegin \DIFadd{bFMA }\DIFaddend feature of TensorCores. 
Note that BQR does require strictly more FLOPs when compared \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend HQR, but these additional FLOPs are negligible \DIFdelbegin \DIFdel{negligible }\DIFdelend in standard precision and \DIFdelbegin \DIFdel{does }\DIFdelend \DIFaddbegin \DIFadd{do }\DIFaddend not impact the numerical stability.
...
In practice, an efficient implementation might require $r$ to be a power of two or a product of small prime factors and result \DIFaddbegin \DIFadd{in }\DIFaddend a thinner $N^{th}$ block compared \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend the rest. 
...
We now present the basic structure for the rounding error analysis for \cref{algo:blockHQR}, which \DIFdelbegin \DIFdel{consist }\DIFdelend \DIFaddbegin \DIFadd{consists }\DIFaddend of: 1) HQR, 2) building the W factor, and 3) updating the remaining blocks with the WY representation.
We have adapted the analysis from \cite{Higham2002} to fit this \DIFdelbegin \DIFdel{exact }\DIFdelend \DIFaddbegin \DIFadd{particular }\DIFaddend variant, and \DIFdelbegin \DIFdel{denote }\DIFdelend \DIFaddbegin \DIFadd{let }\DIFaddend $\hat{\bb{Q}}_{BQR},\hat{\bb{R}}_{BQR}$ \DIFdelbegin \DIFdel{to be }\DIFdelend \DIFaddbegin \DIFadd{denote }\DIFaddend the outputs from \cref{algo:blockHQR}.
...
The upper trapezoidal factor that results from applying HQR to $\bb{C}_{k}^{((k-1)r)}$ corresponds to \DIFdelbegin \DIFdel{the $(k-1)r+1^{st}$ to $kr^{th}$ columns }\DIFdelend \DIFaddbegin \DIFadd{columns $(k-1)r+1$ through $kr$ }\DIFaddend of $\hat{\bb{R}}_{BQR}$, and applying \cref{lem:3.7,lem:19.3} yields
\begin{equation*}
...
\DIFaddbegin \DIFadd{Overdetermined systems with far more rows than columns are called tall-and-skinny.
}\DIFaddend Although Tall-and-Skinny QR (TSQR) broadly refers to block QR factorization methods with row partitions, we will discuss a specific variant of TSQR which is also known as the AllReduce algorithm \cite{Mori2012}.
...
Additionally, our numerical simulations (discussed in \cref{sec:NE}) show that TSQR can not only increase the speed but also \DIFdelbegin \DIFdel{outperform }\DIFdelend \DIFaddbegin \DIFadd{achieve higher accuracy than }\DIFaddend the traditional HQR factorization in low precisions.
...
In the following sections, \cref{algo:par_tsqr} ({\tt tsqr}) will find a TSQR factorization of a matrix \DIFdelbegin \DIFdel{$A\in\R^{m\times n}$ }\DIFdelend \DIFaddbegin \DIFadd{$\bb{A}\in\R^{m\times n}$ }\DIFaddend where $m \gg n$. 
...
\cref{Qdetails} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{Qdetails} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend shows full linear algebra details for a single-level ($L=1$, $2$ initial blocks) example.
...
\end{bmatrix}. \]
More generally, \cref{algo:par_tsqr} takes a tall-and-skinny matrix $\bb{A}$ and level $L$ and finds a QR factorization by initially partitioning $\bb{A}$ into \DIFdelbegin \DIFdel{$2^{(l)}$ }\DIFdelend \DIFaddbegin \DIFadd{$2^{L}$ }\DIFaddend row-blocks and includes the building of $\bb{Q}$.
For simplicity, we assume that $m$ is exactly \DIFdelbegin \DIFdel{$h2^{(l)}$ }\DIFdelend \DIFaddbegin \DIFadd{$h2^{L}$ }\DIFaddend so that the initial partition yields \DIFdelbegin \DIFdel{$2^{(l)}$ }\DIFdelend \DIFaddbegin \DIFadd{$2^{L}$ }\DIFaddend blocks of equal sizes, $h$-by-$n$. 
...
For example, the single-level TSQR decomposition of a tall-and-skinny matrix \DIFdelbegin \DIFdel{, }\DIFdelend $\bb{A}$ requires 3 total HQRs of matrices of sizes $\lfloor\log_{2}(\frac{m}{n})\rfloor$-by-$n$, $\lceil\log_{2}(\frac{m}{n})\rceil$-by-$n$, and $2n$-by-$n$.
...
This case exemplifies a situation in which \DIFdelbegin \DIFdel{stability }\DIFdelend \DIFaddbegin \DIFadd{high accuracy }\DIFaddend is not guaranteed in HQR, but \DIFdelbegin \DIFdel{the method is stable }\DIFdelend \DIFaddbegin \DIFadd{it is guaranteed }\DIFaddend when using TSQR\DIFdelbegin \DIFdel{, even in the worst-case}\DIFdelend . 
Note that \DIFdelbegin \DIFdel{this likely is not the typical situation in practice
.
}\DIFdelend \DIFaddbegin \DIFadd{these worst-case bounds are likely overestimates in practice
...
Figure~\ref{fig:paramspace} shows the matrix sizes HQR, 2-level TSQR, and 4-level TSQR can accommodate as well as their respective error bounds \DIFaddbegin \DIFadd{on the $\bb{Q}$ factor}\DIFaddend .\par
\begin{wrapfigure}{l}{.45\textwidth}
	\centering
	%\vspace{-15pt}
	%\includegraphics[width=.45\textwidth]{./figures/paramspace.png}
	\DIFdelbegin %DIFDELCMD < \includegraphics[width=.45\textwidth]{./paramspace2.png}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \includegraphics[width=.45\textwidth]{../figures/paramspace3.png}
	\DIFaddend \caption{\label{fig:paramspace} Non-white space indicates allowable matrix sizes for each scheme, and color map represents error bounds for $\|\bb{\Delta Q}\|_F$ for uniform precision error analysis when using double precision arithmetic in $\log_{10}$ scale.}
	\vspace{-10pt}	
\end{wrapfigure}
...
\begin{theorem}
	...
	\end{align}
\end{theorem}
\DIFdelbegin \DIFdel{Unsurprisingly, the inner product mixed precision setting yields higher error bounds (see \mbox{%DIFAUXCMD
\cref{thm:feHQR}}\hspace{0pt}%DIFAUXCMD
)as it uses more low precision arithmetic than the settings described in \mbox{%DIFAUXCMD
\cref{sec:mp-3}}\hspace{0pt}%DIFAUXCMD
. 
}\DIFdelend \DIFaddbegin 

\DIFaddend Before commenting on the significance of \cref{thm:mpHQR}, we show that the same bounds hold for the BQR variant.
...
\DIFdelbegin \DIFdel{The corresponding error bound for }%DIFDELCMD < {\tt %%%
\DIFdel{mpBQR3}%DIFDELCMD < } %%%
\DIFdel{of \mbox{%DIFAUXCMD
\cref{sec:mp-3b} }\hspace{0pt}%DIFAUXCMD
yielded low precision errors $r$ times smaller than that from 
using \mbox{%DIFAUXCMD
\cref{assump:mp} }\hspace{0pt}%DIFAUXCMD
inner products, an unsurprising result as intermediate results are cast down more often in }%DIFDELCMD < {\tt %%%
\DIFdel{mpBQR2}%DIFDELCMD < }%%%
\DIFdel{.
%DIF < Furthermore, the $\tilde{\gamma}^{(l)}$ in this section requires $c=\cO(14)$, whereas the same notation in \cref{sec:mp-3b} assumes $c$ to be a \emph{small} positive integer.
...
Therefore, guarantees of numerical stability of }%DIFDELCMD < {\tt %%%
\DIFdel{mpBQR2}%DIFDELCMD < } %%%
\DIFdel{are limited to smaller matrix sizes when compared to those of }%DIFDELCMD < {\tt %%%
\DIFdel{mpBQR3}%DIFDELCMD < } %%%
\DIFdel{and BQR in high precision.
While it is technically possible that the low precision errors introduced from utilizing \mbox{%DIFAUXCMD
\cref{assump:mp} }\hspace{0pt}%DIFAUXCMD
do not dominate the errors incurred in }%DIFDELCMD < {\tt %%%
\DIFdel{mpBQR2}%DIFDELCMD < } %%%
\DIFdel{and }%DIFDELCMD < {\tt %%%
\DIFdel{mpHQR2}%DIFDELCMD < } %%%
\DIFdel{when $m\gg M_{l,h}$ and can result in accuracy comparable to that of }%DIFDELCMD < {\tt %%%
\DIFdel{mpBQR3}%DIFDELCMD < } %%%
\DIFdel{and high precision BQR, our numerical results in \mbox{%DIFAUXCMD
\cref{sec:NE} }\hspace{0pt}%DIFAUXCMD
show that }%DIFDELCMD < {\tt %%%
\DIFdel{mpHQR2}%DIFDELCMD < } %%%
\DIFdel{is already unstable at $m\approx M_{l,h}$.
}\DIFdelend \subsubsection{TSQR round down at inner product: {\tt mpTSQR2}}
...
\end{enumerate}
TensorCore's bFMA \DIFdelbegin \DIFdel{computes 
}\DIFdelend \DIFaddbegin \DIFadd{can compute 
...
\end{equation}
and \DIFdelbegin \DIFdel{employs }\DIFdelend \DIFaddbegin \DIFadd{employ }\DIFaddend \emph{full} precision products and fp32 summation accumulate.
\DIFaddbegin \DIFadd{Note that the latest A100 GPUs are not restricted to fp16/fp32 in the TensorCore instructions \mbox{%DIFAUXCMD
\cite{nvdiaa100}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend Here, the \emph{full} precision multiplication is exact as explained in \cref{sec:background}.
...
Each block is \DIFdelbegin \DIFdel{casted }\DIFdelend \DIFaddbegin \DIFadd{cast }\DIFaddend up to compute a high precision HQR and to form the WY representation. 
The WY representation is then \DIFdelbegin \DIFdel{casted }\DIFdelend \DIFaddbegin \DIFadd{cast }\DIFaddend down to low precision since the bFMAs require low precision inputs for matrix products, and the $\bb{R}$ factor from the high precision HQR can be \DIFdelbegin \DIFdel{casted }\DIFdelend \DIFaddbegin \DIFadd{cast }\DIFaddend down to return a low precision $\bb{R}$ factor at the very end. 
...
\begin{algorithm2e}
	...
	\Return $\bb{Q},\bb{A}$
	\caption{\label{algo:mpBQR} $\hat{\bb{Q}}_{mpBQR3},\hat{\bb{R}}_{mpBQR3}\gets {\tt mpBQR3}(\bb{A}, r)$: Perform a mixed precision variant of BQR \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend low precision $\bb{A}$ with column partitions of size $r$. $\hat{\bb{Q}}_{mpBQR3}$,$\hat{\bb{R}}_{mpBQR3}$, are returned in low precision. Operations in lines 7 and 10 require low precision inputs.}
\end{algorithm2e}

Since $\hat{\bb{W}}_{k},\hat{\bb{Y}}_k$'s \DIFdelbegin \DIFdel{are computed with \mbox{%DIFAUXCMD
\cref{algo:buildWY} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{in \mbox{%DIFAUXCMD
\cref{algo:buildWY} }\hspace{0pt}%DIFAUXCMD
are computed }\DIFaddend in high precision \DIFaddbegin \DIFadd{and }\DIFaddend then cast down, the new low precision WY update is $\hat{\bb{X}}_{k}^{(l)} = \bb{I}-\bb{I}^{(l)}\hat{\bb{W}}_k\bb{I}^{(l)}\hat{\bb{V}}_k^{(\top)}$.
Consider applying $\hat{\bb{X}}_k^{(l)}$ to some matrix \DIFaddbegin \DIFadd{$\bb{B}$ }\DIFaddend stored in low precision \DIFdelbegin \DIFdel{, $\bb{B}$ }\DIFdelend using the TensorCore bFMAs.
...
\end{align*}
Converting to a normwise error bound using the same logic from \cref{eqn:applyP,eqn:19.2c} \DIFdelbegin \DIFdel{, we result }\DIFdelend \DIFaddbegin \DIFadd{results }\DIFaddend in
\begin{equation}
...
\begin{table}[H]
 ...
 BQR& $n^{3/2}\tilde{\gamma}_{m}$ & \DIFdelbeginFL \DIFdelFL{$n^{1/2}\tilde{\gamma}_{10N}^{(l)}+n^{3/2}\tilde{\gamma}_{m}^{(h)}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$n^{1/2}\left(\tilde{\gamma}_{10N}^{(l)}+n\tilde{\gamma}_{m}^{(h)}\right)$ }\DIFaddendFL & $n^{1/2}\left(\tilde{\gamma}_{N}^{(l)} + n\tilde{\gamma}_m^{(h)}\right)$ \\\hline
 TSQR & $n^{3/2}(\tilde{\gamma}_{m2^{-L}}+L\tilde{\gamma}_{2n})$ &  \begin{tabular}{@{}c@{}}\DIFdelbeginFL \DIFdelFL{$n^{1/2}\tilde{\gamma}_{10(L+1)}^{(l)}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$n^{1/2}\left[\right.\tilde{\gamma}_{10(L+1)}^{(l)}$}\DIFaddendFL \\ \DIFdelbeginFL \DIFdelFL{$+n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{m2^{-L}}^{(h)}\right)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$+n\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{m2^{-L}}^{(h)}\right)\left.\right]$}\DIFaddendFL \end{tabular} & \begin{tabular}{@{}c@{}}\DIFdelbeginFL \DIFdelFL{$n^{1/2}\tilde{\gamma}_{L+1}^{(l)}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$n^{1/2}\left[\right.\tilde{\gamma}_{L+1}^{(l)}$ }\DIFaddendFL \\ \DIFdelbeginFL \DIFdelFL{$+n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{2^{-L}m}^{(h)}\right)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$+n\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{2^{-L}m}^{(h)}\right)\left.\right]$}\DIFaddendFL \end{tabular}\\
 \hline
\end{tabular}
	\caption{Error bounds for $\|\Delta \bb{Q}\|_F$ for all algorithms in \cref{sec:algo,sec:mpanalysis} \DIFaddbeginFL \DIFaddFL{for a matrix with $m$ rows and $n$ columns}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{The TSQR algorithms are performed with $L$ levels, and the BQR algorithms are performed with $N$ blocks. Low precision is denoted with $l$, and high precision, $h$.}\DIFaddendFL } 
	\label{table:summary},
\end{table} 
\vspace{-1cm}

\DIFaddbegin \DIFadd{Unsurprisingly, the inner product mixed precision setting from \mbox{%DIFAUXCMD
\cref{sec:mp-2} }\hspace{0pt}%DIFAUXCMD
yields higher error bounds (see \mbox{%DIFAUXCMD
\cref{thm:feHQR}}\hspace{0pt}%DIFAUXCMD
) as it uses more low precision arithmetic than the settings used in \mbox{%DIFAUXCMD
\cref{sec:mp-3}}\hspace{0pt}%DIFAUXCMD
. 
For example, the error bound for }{\tt \DIFadd{mpBQR3}} \DIFadd{of \mbox{%DIFAUXCMD
\cref{sec:mp-3b} }\hspace{0pt}%DIFAUXCMD
yielded low precision errors $r$ times smaller than that of }{\tt \DIFadd{mpBQR2}} \DIFadd{of \mbox{%DIFAUXCMD
\cref{sec:mp-2} }\hspace{0pt}%DIFAUXCMD
, as intermediate results are cast down more often in }{\tt \DIFadd{mpBQR2}}\DIFadd{.
Therefore, guarantees of numerical stability of }{\tt \DIFadd{mpBQR2}} \DIFadd{are limited to smaller matrix sizes when compared with those of }{\tt \DIFadd{mpBQR3}} \DIFadd{and BQR in high precision.
While it is technically possible that the low precision errors introduced from utilizing \mbox{%DIFAUXCMD
\cref{assump:mp} }\hspace{0pt}%DIFAUXCMD
do not dominate the errors incurred in }{\tt \DIFadd{mpBQR2}} \DIFadd{and }{\tt \DIFadd{mpHQR2}} \DIFadd{when $m\gg M_{l,h}$ and can result in accuracy comparable to that of }{\tt \DIFadd{mpBQR3}} \DIFadd{and high precision BQR, our numerical results in \mbox{%DIFAUXCMD
\cref{sec:NE} }\hspace{0pt}%DIFAUXCMD
show that }{\tt \DIFadd{mpHQR2}} \DIFadd{is already unstable at $m\approx M_{l,h}$.
}

\DIFaddend \section{Numerical Experiments}\label{sec:NE}
%\input{NE2}
We conducted several numerical experiments to confirm the validity of the error bounds formed in \cref{sec:mpanalysis} by varying \DIFaddbegin \DIFadd{matrix }\DIFaddend size for all algorithms, block sizes in {\tt mpBQR3}, and comparing {\tt mpHQR2} against {\tt mpTSQR2} with varying condition numbers.
...
\begin{figure}[h!]
	...
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.45\textwidth]{./fixedn.png}
%DIFDELCMD < 	\includegraphics[width=0.45\textwidth]{./figures/mpBQR3-blocksize-1108.png}
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF > \includegraphics[width=0.45\textwidth]{../figures/fixedn_a.png}
	%DIF > \includegraphics[width=0.45\textwidth]{../figures/mpBQR3-blocksize-1108c.png}
	\DIFaddbeginFL \includegraphics[width=\textwidth]{../figures/fig2a.png}
	\vspace{-22pt}
	\DIFaddendFL \caption{\label{fig:sizemp3}Left plot: Backward errors of HH QR factorization algorithms in \cref{sec:algo,sec:mpanalysis} with varying matrix sizes.
...
Figure~\ref{fig:allTSQR} reveals two different trends for the errors as we deepen the complexity of the QR algorithm from {\tt mpHQR2} to {\tt mpTSQR2} with $L=5$\DIFaddbegin \DIFadd{, and these two trends are separated into the center and right plots}\DIFaddend . 
...
\DIFaddbegin \DIFadd{To make these two trends clear, we have used green segments to show when a TSQR with one more level yields smaller errors, and blue segments to show when a TSQR with another level yields larger errors.
}\DIFaddend In these cases, errors from {\tt mpTSQR2} with 3 or more levels are similar to or worse than their 2-level variants, but generally do not exceed those of {\tt mpHQR2} most of the times.
...
\begin{figure}[h!]%{r}{.53\textwidth}
	\centering
	%\vspace{-10pt}
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{./figures/allTSQR3.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figures/allTSQR3.pdf}
	\DIFaddendFL \vspace{-15pt}
	\caption{\label{fig:allTSQR} All plots show the backward relative error for 4000-by-100 sized test matrices. Left: {\tt mpHQR2} on condition numbers ranging from 1.1 to 101;  Middle: {\tt mpTSQR2} on condition numbers ranging from 5.3 to 101; Right:  {\tt mpTSQR2} on condition numbers ranging from 1.1 to 5.3.  \DIFaddbeginFL \DIFaddFL{The red line on both plots corresponds to value }{\tt \DIFaddFL{5.2e-3}}\DIFaddFL{,and the green/blue segments show a decrease/increase in error in comparison to using TSQR with one fewer level. }\DIFaddendFL }
	\vspace{-5pt}
\end{figure}
...
The authors are grateful to the anonymous reviewers for their constructive criticisms, which \DIFdelbegin \DIFdel{imporved }\DIFdelend \DIFaddbegin \DIFadd{improved }\DIFaddend the clarity of the presentation.
\bibliographystyle{siamplain}%ieeetr
\DIFdelbegin %DIFDELCMD < \bibliography{../../../../../library.bib,../../../../../sans_library.bib,./report.bib}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{../../../../../../library.bib,../../../../../../sans_library.bib,./report.bib}
\DIFaddend 
