\begin{table}[H]
	...
	\begin{tabular}{|m{2.cm}|m{10.6cm}|c|}
		...
		$\|{\bf x}\|_2$, $\|\bb{A}\|_2$ & Vector, operator $p$-norms for $p=2$ & \ref{}\\
		$|\bb{x}|$ ,$|\bb{A}|$ & Absolute value of each component of vector, matrix & \ref{} \\
		$\bb{x}[i], \bb{A}[i,j]$,$\;:$& $i^{th}$ element of $\bb{x}$, $i^{th}$ row and $j^{th}$ column element of $\bb{A}$, all indices& \ref{}\\
		...
		\hline
		$\bb{Q},\bb{R}$  & Factors resulting from QR factorization algorithms  & \ref{sec:background}\\
		$\bb{X}, \bb{W}, \bb{Y}$ & WY representation of Householder transformation, $\bb{X}=\bb{I}-\bb{W}\bb{Y}^{\top}$ & \\
