...
While base, precision, and exponent range are fixed and define a floating point number:
\vspace{.2cm}
\begin{center}
...
%%%\end{table}
\vspace{.2cm}
the sign, significand, and exponent identifies a unique number within that system.
...
\subsubsection{Rounding Error Example for the Inner Product}\label{sec:backgroundRE}
...
Since vectors of length $m$ accumulate rounding errors that are bounded by $\gamma_{m}$, the dot products of vectors computed in fp16 already face a 100\% relative error bound in the worst-case scenario ($\gamma_{512}^\text{fp16}=1$). \par
