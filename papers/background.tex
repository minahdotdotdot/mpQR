...
\begin{align*}
	(1+u^l)(1+\gamma_{m-1}^h)-1 &\leq \gamma_{M+m-1}^h=\gamma_{1+(m-1)/M}^l, \;\; M = u^l/u^h\\
	(1+u^l)(1+\gamma_{m-1}^h)-1 &\leq  u^l + \gamma_{m-1}^h + \min\{u^l, \gamma_{m-1}^h\},\;\; \gamma_{m-1}^h < 1
\end{align*}
where the tighter bound can be determined from context.
All of these bounds clearly indicate that this error bounded by $\cO(u^l + mu^h + mu^lu^h)$.
%where $|\Delta \bb{x}| \leq \gamma^{(l)}_{d+1}|\bb{x}|,\;\; |\Delta \bb{y}|  \leq \gamma_{d+1}^{(l)}|\bb{y}| $ componentwise.\par
