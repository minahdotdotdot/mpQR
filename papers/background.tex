...
If $\fl(\bb{D})$ is desired in fp16, then each component of that matrix incur rounding errors as shown in \cref{eqn:aftercd} and if it is desired in fp32, the componentwise rounding error is given by \cref{eqn:beforecd}.
...
In \cref{sec:algo}, we introduce various Householder QR algorithms as well as skeletons for rounding error analysis that we will then modify for different mixed precision assumptions in \cref{sec:mpanalysis}.