...
%	Then,
	...
	\gamma^{(l)}_{k_{l}}+\gamma^{(h)}_{k_{h}}+\gamma^{(l)}_{k_{l}}\gamma^{(h)}_{k_{h}} & < \gamma^{(l)}_{k_{l}+ d_2},% \label{eqn:mp1}
	\end{align*} 
...
The main difference lies in that the FMA block in TensorCore is for matrix-matrix products (level-3 BLAS), but our ad hoc mixed precision inner product FMA defined in \cref{assump:mp} is a level-2 BLAS operation.
Although our analysis concerns accuracy and stability and leaves out timing results of various hardwares, we add a general statement to \cref{assump:mp} that is analogous to that of TensorCore: the mixed precision FMA should perform approximately 2 times faster than in the higher precision.
This gives a vague insight into the trade-offs between speediness and accuracy from some mixed precision computations.  
...
\end{equation}
If we then cast down this result, the forward errors are
\begin{equation}
...
Our error of interest can be bounded in many ways,
\begin{align*}
...
\end{align*}
where the tighter bound can be determined from context, and the constant $M_{l,h}$ represents the disparity in the two precisions. 
Both bounds clearly indicate that this error is bounded by $\cO(u^{(l)} + mu^{(h)} + mu^{(l)}u^{(h)})$.
...
Our analysis in \cref{eqn:aftercd}shows us that the traditional error bound $\gamma_m$ is pushed to the higher precision, $\gamma_{m-1}^{(h)}$, which is a significant reduction from $\gamma_m^{(l)}$ since we can expect $M_{l,h} \gg 1$.
This reduction is penalized by the cast down step, which increases the error bound by approximately $ u^{(l)}$. 
The impact of the cast down step is measured relative to the length of the vector, $m$, since that determines which one out of $u^{(l)}$ and $mu^{(h)}$ is the leading order term. 
There are 3 cases to consider. \\
\textbf{Case 1: ($m\ll M_{l,h}$)} In this case, $u^{(l)}$ is the leading order term, and the mixed precision inner product can claim its superiority in accuracy in comparison to the inner product computed in the lower precision. 
...
In conclusion, this inner product mixed precision successfully reduces the error from $mu^{(l)})$ to $u^{(l)}$ with no apparent improvements in timing.\\
...
Comparing this to the inner product computed entirely in high precision shows that the error has doubled from $mu^{(h)}$ to $2mu^{(h)}$, but gained a factor of 2 speed-up in timing instead. 
One can argue that the loss in accuracy and the improvement in speed have essentially cancelled each other out. \\
\textbf{Case 3: ($m \gg M_{l,h}$)}
Now the second term $\gamma_{m-1}^{(h)}$ is the leading order term. 
As in the above two cases, this is an improvement in the context of the low precision accuracy since the error has been reduced from $\gamma_m^{(l)}$ to $\gamma_{m/M}^{(l)}$. 
Since $u^{(l)} = Mu^{(h)} \ll mu^{(h)}$, the accuracy of the mixed precision inner product procedure from \cref{assump:mp} is what you would expect from carrying the computation out in the higher precision. 
Therefore, we can expect about the same level of accuracy but a factor of 2 reduction in speed when compared to the higher precision.
The remainder of this paper does not discuss improvements in speed and readers should refer to timing studies such as ..... but the above 3 cases cover a portion of the spectrum of possibilities in the current mixed precision computing.
