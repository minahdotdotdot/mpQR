...
%DIF DEL all.tex         Wed Sep 30 07:57:27 2020
%DIF ADD revision2.tex   Mon Oct 12 09:05:32 2020
%Packages
...
\maketitle
\begin{abstract}
	Although mixed precision arithmetic has recently garnered interest for training dense neural networks, many other applications could benefit from the  speed-ups and lower storage \DIFaddbegin \DIFadd{cost }\DIFaddend if applied appropriately. 
...
%\input{intro2}
The accuracy of a numerical algorithm depends on several factors, including numerical stability and well-conditionedness of the problem, both of which may be sensitive to rounding errors, the difference between exact and \DIFdelbegin \DIFdel{finite-precision }\DIFdelend \DIFaddbegin \DIFadd{finite precision }\DIFaddend arithmetic. 
...
However, many applications exist that would benefit from the use of low precision arithmetic and storage that are less sensitive to floating-point \DIFdelbegin \DIFdel{round off }\DIFdelend \DIFaddbegin \DIFadd{round-off }\DIFaddend error, such as clustering or ranking graph algorithms \cite{vonLuxburg2007} or training dense neural networks \cite{micikevicius2018mixed}.
\DIFaddbegin \DIFadd{As a first step towards that goal, we investigate the use of mixed precision arithmetic for the QR factorization, a widely used linear algebra routine.
}\DIFaddend \par
...
Utilizing mixed precision, one can achieve similar quality of computation as \DIFdelbegin \DIFdel{high-precision }\DIFdelend \DIFaddbegin \DIFadd{high precision }\DIFaddend and still achieve 
...
\DIFdelbegin \DIFdel{Trivially, the size and weight of memory required for a specific problem is 4$\times$.
}\DIFdelend %DIF > Trivially, the size and weight of memory required for a specific problem is 4$\times$.
...
For example, Tesla V100's TensorCores perform block Fused Multiply Add operations (bFMAs), where matrix products of fp16 input data can be computed up to $16\times$ \DIFaddbegin \DIFadd{faster }\DIFaddend than that of fp64.
The existing rounding error analyses \DIFaddbegin \DIFadd{of linear algebra routines }\DIFaddend are built within what we call a \emph{uniform precision} setting, which is the assumption that all arithmetic operations and storage are performed via the same precision.
...
In many applications, \DIFdelbegin \DIFdel{computing the }\DIFdelend \DIFaddbegin \DIFadd{only a }\DIFaddend \emph{thin} decomposition \DIFdelbegin \DIFdel{requires }\DIFdelend \DIFaddbegin \DIFadd{is needed as it produces an orthonormal basis for the column space of $\bb{A}$ while requiring }\DIFaddend less computation and \DIFdelbegin \DIFdel{is sufficient in performance.
}\DIFdelend \DIFaddbegin \DIFadd{storage.
...
%DIF > and is sufficient in performance. 
}\DIFaddend While important definitions are stated explicitly in the text, Table~\ref{table:notation} serves to establish basic notation.
%\input{deftable}
\begin{table}[H]
	\centering
...
A simple numerical experiment shows that the standard deterministic error bound is too pessimistic and cannot be practically used to approximate rounding error for \DIFdelbegin \DIFdel{half-precision }\DIFdelend \DIFaddbegin \DIFadd{half precision }\DIFaddend arithmetic. 
...
We see that the inner products of vectors sampled from the standard normal distribution have \DIFdelbegin \DIFdel{backward }\DIFdelend relative errors that do not deviate much from the unit round-off ($\cO$({\tt 1e-4})), whereas the vectors sampled from the uniform distribution tend to accumulate larger errors on average ($\cO$({\tt 1e-3})). 
...
\end{cases} .
\end{equation}
If \DIFdelbegin \DIFdel{$\rm{max}_{(j,k)} u \leq \frac{1}{2}$ }\DIFdelend \DIFaddbegin \DIFadd{$\rm{max}(j,k) u \leq \frac{1}{2}$ }\DIFaddend and $n \leq \frac{1}{uk}$, the operations on the bounds, $\gamma$'s, are:
	\begin{align*}
...
\DIFdelbegin \DIFdel{Although our analysis concerns }\DIFdelend \DIFaddbegin \DIFadd{Our analysis is concerned with }\DIFaddend accuracy and stability and leaves out timing results of various hardwares\DIFdelbegin \DIFdel{, we add a general timing statement to \mbox{%DIFAUXCMD
\cref{assump:mp} }\hspace{0pt}%DIFAUXCMD
that is analogous to that of TC: the mixed precision FMA inner product performs at least 2 times faster than the inner product in the higher precision.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > , we add a general timing statement to \cref{assump:mp} that is analogous to that of TC: the mixed precision FMA inner product performs at least 2 times faster than the inner product in the higher precision.
}\DIFaddend Note that TCs perform matrix-matrix multiply and accumulate up to 8 times faster than fp32, and up to 16 times faster than fp64 (see \cite{Markidis2018})\DIFdelbegin \DIFdel{, and our ad hoc timing assumption is in conservative in comparison.
...
%DIF > , and our ad hoc timing assumption is conservative in comparison. 
%DIF > Nonetheless, this gives a vague insight into the trade-offs between speediness and accuracy from some mixed precision computations.
}\DIFaddend 

\DIFdelbegin \DIFdel{The full precision multiplication in Assumption~\ref{assump:mp} is exact }\DIFdelend %DIF > The full precision multiplication in \cref{assump:mp} is exact when the low precision type is fp16 and the high precision type of fp32 due to their precisions and exponent ranges.
\DIFaddbegin \DIFadd{The exact product in \mbox{%DIFAUXCMD
\cref{assump:mp} }\hspace{0pt}%DIFAUXCMD
is the result of using full precision products }\DIFaddend when the low precision type is fp16 and the high precision type \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend fp32 \DIFdelbegin \DIFdel{due to their precisions and exponent ranges}\DIFdelend \DIFaddbegin \DIFadd{as is in TC bFMAs}\DIFaddend .
...
where  $\mu_x\mu_y \in[0,(2^{11}-1)^2] \subseteq [0,2^{24}-1]$ and $\eta_x+\eta_y +2\in[-28,34]\subseteq[-127,128]$\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{.
Thus, when two fp16 numbers are multiplied and stored in fp32, there is no roundoff error, }\DIFaddend and \DIFdelbegin \DIFdel{therefore is exact.
Thus, }\DIFdelend the summation and the final cast down operations are the only sources of rounding error in this inner product scheme.
\begin{assump}
	\label{assump:mp}
	Let $l$ and $h$ each denote low and high precision types with unit round-off values $u^{(l)}$ and $u^{(h)}$, where $1 \gg u^{(l)} \gg u^{(h)} >0$.
	Consider an FMA operation for inner products that take vectors stored in precision $l$, compute products \DIFdelbegin \DIFdel{in full precision}\DIFdelend \DIFaddbegin \DIFadd{exactly}\DIFaddend , and sum the products in precision $h$. 
	Finally, the result is then cast back down to precision $l$.
	\DIFdelbegin \DIFdel{Furthermore, we expect this procedure to be approximately twice as fast as if it were done entirely in the higher precision, and about the same as if it were done entirely in the lower precision. 
}\DIFdelend %DIF > Furthermore, we expect this procedure to be approximately twice as fast as if it were done entirely in the higher precision, and about the same as if it were done entirely in the lower precision. 
\end{assump}
...
Let \DIFdelbegin \DIFdel{$\bb{x}^{(l)},\bb{y}^{(l)}$ }\DIFdelend \DIFaddbegin \DIFadd{$\bb{x},\bb{y}$ }\DIFaddend be $m$-length vectors stored in a low precision type ($\F_l^m$), $s_k$ be the exact $k^{th}$ partial sum, and \DIFdelbegin \DIFdel{$\hat{s_k}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{s}_k$ }\DIFaddend be $s_k$ computed with FLOPs.
Then the first three partial sums are,
\begin{align*}
\hat{s}_1 &= \fl (\bb{x}[1]\bb{y}[1]) = \bb{x}[1]\bb{y}[1],\quad \hat{s}_2 = \fl(\DIFdelbegin %DIFDELCMD < \hat{s_1} %%%
\DIFdelend \DIFaddbegin \hat{s}\DIFadd{_1 }\DIFaddend + \bb{x}[2]\bb{y}[2]) = \left(\bb{x}[1]\bb{y}[1]+ \bb{x}[2]\bb{y}[2]\right)(1+\dd_{1}^{(h)}),\\
\hat{s}_3 &= \fl(\DIFdelbegin %DIFDELCMD < \hat{s_2}%%%
\DIFdelend \DIFaddbegin \hat{s}\DIFadd{_2}\DIFaddend +\bb{x}[3]\bb{y}[3]) = \left[\left(\bb{x}[1]\bb{y}[1] + \bb{x}[2]\bb{y}[2]\right)(1+\dd_{1}^{(h)})  + \bb{x}[3]\bb{y}[3]\right](1+\dd_{2}^{(h)}).
\end{align*}
...
The mixed precision inner product has a smaller worst case error bound than the bound of the low precision inner product ($mu^{(l)}$)\DIFdelbegin \DIFdel{with no apparent improvements in speed.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  with no apparent improvements in speed.
}\DIFaddend On the other hand, $u^{(l)}$ is a larger upper bound than that of the high precision inner product ($mu^{(h)}=\frac{m}{M_{l,h}}u^{(l)}$)\DIFdelbegin \DIFdel{, although it was computed approximately twice as fast.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > , although it was computed approximately twice as fast. 
}\DIFaddend It is likely that this factor of $M_{l,h}/m$ increase in the worst case error bound is unwanted\DIFdelbegin \DIFdel{even when considering the speed-up.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  even when considering the speed-up.
}\DIFaddend \\
...
Comparing this to the high precision inner product shows that the error bound has doubled from $mu^{(h)}$ to $2mu^{(h)}$\DIFdelbegin \DIFdel{, but gained a factor of 2 in speed instead.
One can argue that the loss in accuracy guarantee and the improvement in speed cancel each other out especially if $2mu^{(h)}\ll 1$ or if the speed-up greatly exceeds a factor of 2. }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > , but gained a factor of 2 in speed instead. 
%DIF > One can argue that the loss in accuracy guarantee and the improvement in speed cancel each other out especially if $2mu^{(h)}\ll 1$ or if the speed-up greatly exceeds a factor of 2. \\
}\DIFaddend \textbf{Case 3: ($m \gg M_{l,h}$)}
...
Therefore, we can expect about the same level of accuracy\DIFdelbegin \DIFdel{but a factor of 2 or greater reduction in speed when compared to the higher precision}\DIFdelend .\par
\DIFdelbegin \DIFdel{While the above cases establish 3 regimes of trade-offs between accuracy and speed in mixed precision computing, the remainder of this paper focuses only on accuracy and does not consider the impact of mixed precision computations on speed.
}\DIFdelend %DIF >  but a factor of 2 or greater reduction in speed when compared to the higher precision.\par
%DIF > While the above cases establish 3 regimes of guarantees for accuracy in mixed precision computing, the remainder of this paper focuses only on accuracy and does not consider the impact of mixed precision computations on speed.
...
\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{Using these }\DIFaddend two mixed precision settings \DIFdelbegin \DIFdel{add }%DIFDELCMD < {\tt %%%
\DIFdel{castdown}%DIFDELCMD < } %%%
\DIFdel{operations }\DIFdelend \DIFaddbegin \DIFadd{in HQR algorithms results in casting down to the low precision }\DIFaddend at different parts of the \DIFdelbegin \DIFdel{HQR algorithmswe consider.
}\DIFdelend \DIFaddbegin \DIFadd{algorithms.
%DIF > The two mixed precision settings add {\tt castdown} operations at different parts of the HQR algorithms we consider.
}\DIFaddend In general, error bounds in the fashion of \cref{eqn:beforecd} \DIFdelbegin \DIFdel{can be used before the }\DIFdelend \DIFaddbegin \DIFadd{correspond to rounding errors prior to }\DIFaddend cast down operations\DIFdelbegin \DIFdel{and the action of the cast down is best represented by error bounds similar to }\DIFdelend \DIFaddbegin \DIFadd{, and cast down operations introduce an additional error term as in }\DIFaddend \cref{eqn:aftercd}.\par
...
\begin{lemma}
	Given vector $\bb{x}\in\R^{m}$, there exist \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH vector , $\bb{v}$, and \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH constant, $\beta$, that define the HH transformation matrix, $\bb{P}_{\bb{v}}:=\bb{I}_{m} - \beta \bb{v}\bb{v}^{\top}$, such that $\bb{P}_{\bb{v}}$ \DIFdelbegin \DIFdel{zeros }\DIFdelend \DIFaddbegin \DIFadd{zeroes }\DIFaddend out $\bb{x}$ below the first element. 
...
\begin{enumerate}[Step 1)]
	\item Compute $\bb{v}$ and $\beta$ that \DIFdelbegin \DIFdel{zeros }\DIFdelend \DIFaddbegin \DIFadd{zeroes }\DIFaddend out the $i^{th}$ column of $\bb{A}$ beneath $a_{ii}$ (see \cref{algo:hh_v2}), and
...
For now, consider zeroing out any vector $\bb{x}\in\R^m$ below its first component with \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH transformation.
...
\end{equation}
and $|\Delta {\bf v}[1]| = 0$.
Since \DIFdelbegin \DIFdel{$1+2(m+2) += \cO(m)$}\DIFdelend \DIFaddbegin \DIFadd{$1+2(m+2) = \cO(m)$}\DIFaddend , we have swept that minor difference \DIFdelbegin \DIFdel{between }\DIFdelend under our use of the $\tilde{\gamma}$ notation defined in \cref{lem:gamma}.
...
&= (1+\tth\DIFdelbegin \DIFdel{_{3m+5}}\DIFdelend \DIFaddbegin \DIFadd{_{2m+4}}\DIFaddend )\beta= \beta + \Delta \beta,\text{ where } |\Delta\beta| \leq \tilde{\gamma}_{m} \beta\label{eqn:beta3}.
\end{align}
We have shown \cref{eqn:beta} to keep our analysis simple in \cref{sec:mpanalysis} and \cref{eqn:beta3} \DIFaddbegin \DIFadd{to }\DIFaddend show that the error incurred from calculating \DIFdelbegin \DIFdel{of }\DIFdelend $\|\bb{x}\|_2$ accounts for the vast majority of the rounding error so far.
...
Therefore, we now need to calculate the errors for applying \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH transformation to the remaining columns, $\bb{A}[i:m, i+1:n]$ with the computed HH vector and constant.
...
We show the error for forming $\hat{\bb{w}}$ where \DIFdelbegin \DIFdel{$\bb{w}:=\beta\bb{v}^{\top}\bb{x}\bb{v}$ }\DIFdelend \DIFaddbegin \DIFadd{$\bb{w}:=\beta(\bb{v}^{\top}\bb{x})\bb{v}$ }\DIFaddend and $\bb{v},\bb{x}\in\R^{m}$,
\begin{equation*}
\hat{\bb{w}} =\fl(\hat{\beta}\;\fl(\hat{\bb{v}}^{\top}\bb{x})\DIFdelbegin %DIFDELCMD < \hat{v}%%%
\DIFdelend \DIFaddbegin \hat{\bb{v}}\DIFaddend )=(1+\tth_{m})(1+\dd)(1+\dd')(\beta+\Delta\beta)\DIFaddbegin \left(\DIFaddend (\bb{v}+\Delta \bb{v})^{\top}\bb{x}\DIFaddbegin \right)\DIFaddend (\bb{v}+\Delta \bb{v}),
\end{equation*}
where $\tth_{m}$ is from computing the inner product \DIFdelbegin \DIFdel{$\hat{\bb{v}}^{\top}\bb{x}$}\DIFdelend \DIFaddbegin \DIFadd{$(\hat{\bb{v}}^{\top}\bb{x})$}\DIFaddend , and $\dd$ and $\dd'$ are from multiplying $\beta$, $\fl(\hat{\bb{v}}^{\top}\bb{x})$, and $\bb{\hat{v}}$.
The forward error is
$\hat{\bb{w}} = \bb{w} + \Delta \bb{w}$, where \DIFdelbegin \DIFdel{$|\Delta \bb{w}| \leq \tilde{\gamma}_m|\beta||\bb{v}|^{\top}|\bb{x}||\bb{v}|.$
}\DIFdelend \DIFaddbegin \DIFadd{$|\Delta \bb{w}| \leq \tilde{\gamma}_m|\beta|\left(|\bb{v}|^{\top}|\bb{x}|\right)|\bb{v}|.$
}\DIFaddend Subtracting $\hat{\bb{w}}$ from $\bb{x}$ yields the HH transformation with forward error,
\begin{equation}
...
\begin{lemma}\label{lem:3.7}
	If $\bb{X}_{j} + \Delta \bb{X}_{j} \in\R^{m\times m}$ satisfies \DIFdelbegin \DIFdel{$\|\Delta \bb{X}_{j}\|_F\leq \dd_j \|\bb{X}_{j}\|_2$ for all $j$, then }\DIFdelend \DIFaddbegin \DIFadd{$\|\Delta \bb{X}_{j}\|_F\leq \tau_j \|\bb{X}_{j}\|_2$ for all $j=1,\cdots,r$, then }\DIFaddend $$\left|\left|\prod_{j=1}\DIFdelbegin \DIFdel{^n }\DIFdelend \DIFaddbegin \DIFadd{^r }\DIFaddend (\bb{X}_{j} + \Delta \bb{X}_{j})- \prod_{j=1}\DIFdelbegin \DIFdel{^n}\DIFdelend \DIFaddbegin \DIFadd{^r}\DIFaddend \bb{X}_{j} \right|\right|_F\leq\left(-1+\prod_{j=1}\DIFdelbegin \DIFdel{^n}\DIFdelend \DIFaddbegin \DIFadd{^r}\DIFaddend (1+\DIFdelbegin \DIFdel{\dd}\DIFdelend \DIFaddbegin \DIFadd{\tau}\DIFaddend _j)\right)\prod_{j=1}^n\|\bb{X}_{j}\|_2.$$
\end{lemma}
...
\begin{proof}
	...
	\[\DIFdelbegin %DIFDELCMD < \scriptstyle
%DIFDELCMD < 		%%%
\DIFdelend 
...
\end{proof}
In this error analysis, the prevailing bound for errors at various stages of forming and applying \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH transformation is $\tilde{\gamma}_{m}$ where $m$ corresponds to the dimension of the transformed vectors.
...
\Cref{lem:WY} shows how to update a rank-$j$ update of the identity, $\bb{Q}^{(j)}$, with \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH transformation, $\bb{P}$, to produce a rank-$(j+1)$ update of the identity, $\bb{Q}^{(j+1)}$. 
...
In HQR, $\bb{A}$ is transformed into an upper triangular matrix $\bb{R}$ by identifying \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH transformation that \DIFdelbegin \DIFdel{zeros }\DIFdelend \DIFaddbegin \DIFadd{zeroes }\DIFaddend out a column below the diagonal, then applying that HH transformation to the bottom right partition. 
For example, the $k^{th}$ HH transformation finds an $m-k+1$ sized HH transformation that \DIFdelbegin \DIFdel{zeros }\DIFdelend \DIFaddbegin \DIFadd{zeroes }\DIFaddend out column $k$ below the diagonal and then applies it to the $(m-k+1)$-by-$(n-k)$ partition of the matrix, $\bb{A}[k:m,k+1:n]$.
Since the \DIFdelbegin \DIFdel{$k+1^{st}$ }\DIFdelend \DIFaddbegin \DIFadd{$(k+1)^{th}$ }\DIFaddend column is transformed by the $k^{th}$ HH transformation, this algorithm must be executed serially as shown in \cref{algo:hhQR}.
...
BQR performs approximately $1-\cO(1/N)$ fraction of its FLOPs in Level-3 BLAS operations (see section 5.2.3 of \cite{golub2013matrix}), and can reap the benefits from the accelerated block FMA feature of \DIFdelbegin \DIFdel{TensorCore}\DIFdelend \DIFaddbegin \DIFadd{TensorCores}\DIFaddend . 
Note that BQR does require strictly more FLOPs when compared to HQR, but these additional FLOPs are \DIFdelbegin \DIFdel{negligble }\DIFdelend \DIFaddbegin \DIFadd{negligible negligible }\DIFaddend in standard precision and does not impact the numerical stability.
...
\begin{equation}
\DIFdelbegin %DIFDELCMD < \hat{\bb{X}_k^{(j)}} %%%
\DIFdelend \DIFaddbegin \hat{\bb{X}}\DIFadd{_k^{(j)} }\DIFaddend = \hat{\bb{X}}_k^{(j-1)} - \fl(\hat{\beta}_k^{(j-1)}\hat{\bb{X}}_k^{(j-1)}\hat{\bb{v}}_k^{(j-1)})\hat{\bb{v}}_k^{(j)\top} = \hat{\bb{X}}_k^{(j-1)}(\bb{P}_k^{(j)}+\Delta \bb{P}_k^{(j)}), \label{eqn:Xupdate}
\end{equation}
...
\begin{equation}
	\|\Delta \bb{X}_k^{(j)} \|_F \DIFaddbegin \DIFadd{:= \|}\hat{\bb{X}}\DIFadd{_k^{(j)}-\bb{X}_k^{(j)}\|_F }\DIFaddend \leq j\tilde{\gamma}_{m-(k-1)r}. \label{eqn:deltX}
\end{equation}
...
\begin{align}
	\|\fl(\hat{\bb{X}}_k \bb{B}[:,j])\|_2 = \|\fl(\hat{\bb{X}}_k^{(r)} \bb{B}[:,j])\|_2 \leq r\tilde{\gamma}_{m-(k-1)r} \|\bb{B}[:,j]\|_2\DIFaddbegin \DIFadd{.
}\DIFaddend \end{align}
...
\begin{algorithm2e}[H]
	...
	\DIFdelbegin \DIFdel{$h \gets m2^{-L}$ }\DIFdelend \DIFaddbegin \DIFadd{$h \gets 2^{-L}m$ }\DIFaddend \tcp*{Number of rows.}
...
\begin{theorem}
	...
	Let us further assume that $m$ is divisible by $2^L$ and \DIFdelbegin \DIFdel{$n\tilde{\gamma}_{m2^{-L}}, n\tilde{\gamma}_{ 2n} \ll 1$}\DIFdelend \DIFaddbegin \DIFadd{$n\tilde{\gamma}_{2^{-L}m}, n\tilde{\gamma}_{ 2n} \ll 1$}\DIFaddend .
	...
	\begin{align}
	\|\hat{\bb{R}}_{TSQR}[:,j]-\bb{R}[:,j]\|_2 &\leq n(\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}} }\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m} }\DIFaddend + L\tilde{\gamma}_{ 2n})\|\bb{A}[:,j]\|_2,  \label{eqn:tsqrRA}\\
	\|\hat{\bb{Q}}_{TSQR}-\bb{Q}\|_F &\leq n^{3/2}(\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}} }\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m} }\DIFaddend + L\tilde{\gamma}_{ 2n}).\label{eqn:tsqrQ}
	%DIF < \|\hat{\bb{Q}}_{TSQR}[:,j]-\bb{Q}[:,]\|_2 &\leq n(\tilde{\gamma}_{m2^{-L}} + L\tilde{\gamma}_{ 2n}).\label{eqn:tsqrQ}
	%DIF > \|\hat{\bb{Q}}_{TSQR}[:,j]-\bb{Q}[:,]\|_2 &\leq n(\tilde{\gamma}_{2^{-L}m} + L\tilde{\gamma}_{ 2n}).\label{eqn:tsqrQ}
	\end{align}
\end{theorem}

Note that the \DIFdelbegin \DIFdel{$n\tilde{\gamma}_{m2^{-L}}$ }\DIFdelend \DIFaddbegin \DIFadd{$n\tilde{\gamma}_{2^{-L}m}$ }\DIFaddend and $n\tilde{\gamma}_{ 2n}$ terms correspond to errors from applying HQR to the blocks in the initial partition and to the blocks in levels 1 through $L$ respectively.
...
\begin{equation*}
1\gg n^{3/2}\gamma\DIFdelbegin \DIFdel{^{(m)} }\DIFdelend \DIFaddbegin \DIFadd{_m }\DIFaddend \gg n^{3/2}(\gamma\DIFdelbegin \DIFdel{^{(\frac{m}{2^L})}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend +L\gamma\DIFdelbegin \DIFdel{^{(2n)}}\DIFdelend \DIFaddbegin \DIFadd{_{2n}}\DIFaddend ).
\end{equation*}
...
\DIFaddbegin \DIFadd{Note that this likely is not the typical situation in practice.
}\DIFaddend Now consider some $2^{20}$-by-$2^{12}$ matrix and QR factorizations performed with double precision.
The error bound for HQR is {\tt 1.686e-7}, whereas the error bound for TSQR with 12 levels is {\tt 5.351e-10}.
In general, we can conjecture that values of $L$ that can make \DIFdelbegin \DIFdel{$m2^{-L}$ }\DIFdelend \DIFaddbegin \DIFadd{$2^{-L}m$ }\DIFaddend and $2Ln$ much smaller than $m$, should produce a TSQR that outperforms HQR in worst-case scenarios, at least in uniform precision settings.
...
\begin{wrapfigure}{l}{.45\textwidth}
	\centering
	%\vspace{-15pt}
	\DIFdelbegin %DIFDELCMD < \includegraphics[width=.45\textwidth]{./figures/paramspace.png}
%DIFDELCMD < 	%%%
\DIFdelend %DIF > \includegraphics[width=.45\textwidth]{./figures/paramspace.png}
	\DIFaddbegin \includegraphics[width=.45\textwidth]{./paramspace2.png}
	\DIFaddend \caption{\label{fig:paramspace} Non-white space indicates allowable matrix sizes for each scheme, and color map represents error bounds for $\|\bb{\Delta Q}\|_F$ for uniform precision error analysis when using double precision arithmetic \DIFaddbegin \DIFadd{in $\log_{10}$ scale}\DIFaddend .}
	\vspace{-10pt}	
\end{wrapfigure}
...
\cite{Blanchard2019}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Blanchard2020}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , the authors investigate all four possible matrix-matrix multiplication routines in \DIFdelbegin \DIFdel{TensorCore}\DIFdelend \DIFaddbegin \DIFadd{TensorCores}\DIFaddend , which depend on whether $\bb{C}$ and $\bb{D}$ are computed in fp16 or fp32. 
...
\end{align*}
Adapting the rounding error analysis in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Blanchard2019} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Blanchard2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend into this specific mixed precision matrix product setting yields the componentwise forward bound 
\begin{equation}
|\bb{Z}-\fl(\bb{Z})| \leq \left(u^{(\text{fp16})}+ \gamma\DIFdelbegin \DIFdel{_{p/4}}\DIFdelend \DIFaddbegin \DIFadd{_{p}}\DIFaddend ^{(\text{fp32})}+u^{(\text{fp16})} \gamma\DIFdelbegin \DIFdel{_{p/4}}\DIFdelend \DIFaddbegin \DIFadd{_{p}}\DIFaddend ^{(\text{fp32})}\right)|\bb{X}||\bb{Y}|.\label{eqn:bFMAerr}
\end{equation}
...
\begin{equation}
	\|\hat{\bb{Q}}_{mpBQR}-\bb{Q}\|_F \leq n^{1/2}\left(\tilde{\gamma}_{N}^{(l)} + n\tilde{\gamma}_m^{(h)}\right),\DIFaddbegin \label{eqn:mpBQR3err}
...
\end{equation}
\DIFdelbegin \DIFdel{Therefore, }\DIFdelend \DIFaddbegin \DIFadd{In practice, we expect hardware specifications to restrict $r$ between $\cO(10)$ and $\cO(100)$ and fix finite options for $u_l$, $u_h$, and therefore $M_{l,h}$.
This helps narrow our analysis. 
In general, }\DIFaddend the loss in accuracy due to mixed precision computing is relatively small when the disparity in precision ($M_{l,h}$) is small in comparison to the block size, $mr$.
\DIFdelbegin \DIFdel{However, as }\DIFdelend \DIFaddbegin \DIFadd{As we can assume }\DIFaddend $r$ \DIFaddbegin \DIFadd{to be fixed at $\cO(100)$ at the largest, the loss of accuracy attributed to mixed precision computing grows small as $m$ }\DIFaddend grows large, \DIFdelbegin \DIFdel{$N=n/r$ decreases which then reduces the portion of }\DIFdelend \DIFaddbegin \DIFadd{indicating that }\DIFaddend {\tt mpBQR3} \DIFdelbegin \DIFdel{performed using level-3 BLAS operations}\DIFdelend \DIFaddbegin \DIFadd{can yield accuracy comparable to its high, uniform precision variant while still benefiting from speed-ups. 
Next, note that $r=1$ }\DIFaddend and \DIFdelbegin \DIFdel{increases the size of high precision HQR being performed at each block.
%DIF < TODO: line T
Whether this loss in accuracy in the worst-case scenario is worth the }\DIFdelend \DIFaddbegin \DIFadd{$r=n$ both revert to HQR with only level-2 BLAS operations, and other values of $r$ determine the proportion of level-2 and level-3 BLAS operations.
%DIF > As $r$ approaches $n$, $N=n/r$ decreases which then reduces the portion of {\tt mpBQR3} performed using level-3 BLAS operations and increases the size of high precision HQR being performed at each block.
The optimal blocksize only needs to be searched within the permissible ranges for $r$ and $M_{l,h}$.
Overall, the actual trade-off between }\DIFaddend speed-ups \DIFaddbegin \DIFadd{and accuracy }\DIFaddend from using mixed precision hardware \DIFaddbegin \DIFadd{for the QR factorization }\DIFaddend is an open question that can be tackled \DIFaddbegin \DIFadd{more comprehensively }\DIFaddend in future research.
Our analysis shows that the \DIFdelbegin \DIFdel{block size }\DIFdelend \DIFaddbegin \DIFadd{worst-case bounds depend on block width }\DIFaddend $r$, the dimension of the input matrix $m,n$, \DIFdelbegin \DIFdel{and hardware specificitieswill be contributing factors}\DIFdelend \DIFaddbegin \DIFadd{as well as hardware specificities}\DIFaddend . 
...
\paragraph{Rounding Error analysis} The analysis in \cite{Mori2012} shows that each column of $\bb{Q}$ is transformed by $n$ HH transformations of length $2n$ from levels $L:-1:1$, and another set of $n$ HH transformations of length \DIFdelbegin \DIFdel{$m2^{-L}$ }\DIFdelend \DIFaddbegin \DIFadd{$2^{-L}m$ }\DIFaddend at level $0$.
...
\begin{equation}
	\|\fl_{TC}(\hat{\bb{X}}_j^{(i)})-\bb{X}_j^{(i)})\|_F \leq \tilde{\gamma}_2^{(l)} +n\tilde{\gamma}_{m'}^{(h)} + n\tilde{\gamma}_2^{(l)}\tilde{\gamma}_{m'}^{(h)}, \;\; m' = \DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD < 	m2^{-L}, &i=0\\
%DIFDELCMD < 	2n, & i = 1 : L
%DIFDELCMD < 	\end{cases}%%%
\DIFdelend \DIFaddbegin \begin{cases}
	...
	\end{cases}\DIFaddend .
\end{equation}
We can now modify the analysis in \cite{Mori2012} by replacing \DIFdelbegin \DIFdel{$n\tilde{\gamma}_{m2^{-L}}$ }\DIFdelend \DIFaddbegin \DIFadd{$n\tilde{\gamma}_{2^{-L}m}$ }\DIFaddend and $n\tilde{\gamma}_{2n}$ with \[(1+\tilde{\gamma}_2^{(l)})(1+n\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)})-1,\quad\text{and}\quad (1+\tilde{\gamma}_2^{(l)})(1+n\tilde{\gamma}_{2n}^{(h)})-1,\]
...
\begin{align*}
\|(\hat{\bb{R}}_{mpTSQR3}- \bb{R})[:,j]\|_2 &\leq \tilde{\gamma}_{L+1}^{(l)}+n\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)}\right)\|\bb{A}[:,j]\|_2\label{eqn:mpTSQR1},\\
	\|\hat{\bb{Q}}_{mpTSQR3} - \bb{Q}\|_F &\leq n^{1/2}\tilde{\gamma}_{L+1}^{(l)}+n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)}\right).
\end{align*}
...
\begin{equation}
	n^{1/2}\tilde{\gamma}_{L+1}^{(l)}+n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)}\right) = \left(1+ \DIFdelbegin \DIFdel{\frac{M_{l,h}(L+1)}{n(2nL+m2^{-L})}}\DIFdelend \DIFaddbegin \DIFadd{\frac{M_{l,h}(L+1)}{n(2^{-L}m+2nL)}}\DIFaddend \right)n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)}\right).\label{eqn:mpTSQR3}
\end{equation}
...
\begin{table}[H]
	...
	\begin{tabular}{||c|c|c|c||} 
		...
		\DIFdelbeginFL \DIFdelFL{$\frac{(L+1)}{n(2nL+m2^{-L})}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\frac{(L+1)}{n(2^{-L}m+2nL)}$}\DIFaddendFL &  $1/(n^2+m/4)$ &$1/\left(2n^2+\frac{m^{1/2}n^{3/2}}{\log_2(m/n)}\right)$ & $1/(2n^2)$\\ \hline
	\end{tabular}
	\caption{Error bounds for $\|\Delta \bb{Q}_{mpTSQR3}\|_F$ for varying $L$'s.} 
	\label{table:mpTSQR3}
\end{table} 
\vspace{-1cm}
\DIFaddbegin \DIFadd{Finally, the error bounds for the $\bb{Q}$ matrix formed from implementing all of the algorithms discussed in \mbox{%DIFAUXCMD
\cref{sec:mpanalysis,sec:algo} }\hspace{0pt}%DIFAUXCMD
are summarized below in \mbox{%DIFAUXCMD
\cref{table:summary}}\hspace{0pt}%DIFAUXCMD
.
}\begin{table}[H]
	\center
	\begin{tabular}{|p{0.75in}||p{1.3in}|p{1.5in}|p{1.5in}|}\hline
 \DIFaddFL{Algorithm}& \DIFaddFL{Uniform }& \DIFaddFL{Mixed BLAS-2 (\S\ref{sec:mp-2}) }& \DIFaddFL{Mixed BLAS-3 (\S\ref{sec:mp-3})}\\
 \hline\hline
 \DIFaddFL{HQR (\S\ref{sec:HQR}) }& \DIFaddFL{$n^{3/2}\tilde{\gamma}_{m}$ }& \DIFaddFL{$n^{1/2}(\tilde{\gamma}_{10n}^{(l)}+n\tilde{\gamma}_{m}^{(h)})$ }& \DIFaddFL{n/a }\\\hline
 \DIFaddFL{BQR}& \DIFaddFL{$n^{3/2}\tilde{\gamma}_{m}$ }& \DIFaddFL{$n^{1/2}\tilde{\gamma}_{10N}^{(l)}+n^{3/2}\tilde{\gamma}_{m}^{(h)}$ }& \DIFaddFL{$n^{1/2}\left(\tilde{\gamma}_{N}^{(l)} + n\tilde{\gamma}_m^{(h)}\right)$ }\\\hline
 \DIFaddFL{TSQR }& \DIFaddFL{$n^{3/2}(\tilde{\gamma}_{m2^{-L}}+L\tilde{\gamma}_{2n})$ }&  \begin{tabular}{@{}c@{}}\DIFaddFL{$n^{1/2}\tilde{\gamma}_{10(L+1)}^{(l)}$}\\ \DIFaddFL{$+n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{m2^{-L}}^{(h)}\right)$}\end{tabular} & \begin{tabular}{@{}c@{}}\DIFaddFL{$n^{1/2}\tilde{\gamma}_{L+1}^{(l)}$ }\\ \DIFaddFL{$+n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{2^{-L}m}^{(h)}\right)$}\end{tabular}\\
 \hline
\end{tabular}
	\caption{\DIFaddFL{Error bounds for $\|\Delta \bb{Q}\|_F$ for all algorithms in \mbox{%DIFAUXCMD
\cref{sec:algo,sec:mpanalysis}}\hspace{0pt}%DIFAUXCMD
.}} 
	\label{table:summary}\DIFaddFL{,
}\end{table} 
\vspace{-1cm}
\DIFaddend %\input{mp2}
\subsection{Round down at inner product: level-2 BLAS mixed precision setting}\label{sec:mp-2}
\DIFdelbegin \DIFdel{While the previous section discussed blocked variants of HQR that can be easily adapted for the mixed precision setting specific to TensorCore bFMA's, we want to provide a more general mixed precision environment in this section.
}\DIFdelend %DIF > While the previous section discussed blocked variants of HQR that can be easily adapted for the mixed precision setting specific to TensorCore bFMA's, we want to provide a more general mixed precision environment in this section.
...
Consider forming \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend HH transformation that \DIFdelbegin \DIFdel{zeros }\DIFdelend \DIFaddbegin \DIFadd{zeroes }\DIFaddend out $\bb{x}\in\R^m$ below the the $i^{th}$ element. 
...
\fl(\beta) &= \beta +\Delta \beta= (1+\dd^{(l)})\left(-\bb{v}'[1]/\hat{\sigma}\right), \;\; |\Delta\beta| \leq (\gamma\DIFdelbegin \DIFdel{_{8}}\DIFdelend \DIFaddbegin \DIFadd{_{5}}\DIFaddend ^{(l)}+\tilde{\gamma}_{m}^{(h)})|\beta|, \label{eqn:mpbeta}\\
\fl(\bb{v}[j])	&= \bb{v}[j] + \Delta \bb{v}[j]\text{ where }|\Delta \bb{v}[j]|\leq 
(\gamma\DIFdelbegin \DIFdel{_{7}}\DIFdelend \DIFaddbegin \DIFadd{_{3}}\DIFaddend ^{(l)} + \tilde{\gamma}_{m}^{(h)})|\bb{v}_j|,j=2:m-i+1 \label{eqn:mpv}. 
\end{align}
...
\begin{equation}
\hat{\bb{y}} = \bb{y}+\Delta \bb{y},\;\; \|\Delta \bb{y}\|_2 \leq (\gamma\DIFdelbegin \DIFdel{_{25}}\DIFdelend \DIFaddbegin \DIFadd{_{14}}\DIFaddend ^{(l)} + \tilde{\gamma}_{m}^{(h)})\|\bb{y}\|_2.\label{eqn:mpdelty}
\end{equation}
Thus, a backward error can be formed using $\Delta \bb{P_v} = \Delta \bb{y}\bb{x}^{
	\top}/\|\bb{x}\|_2^2$ \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{and by introducing the $\tilde{\gamma}$ notation for the low precision term,
}\DIFaddend \begin{equation}
\hat{\bb{y}} = (\bb{P_v} + \Delta \bb{P_v})\bb{x},\;\; \|\Delta \bb{P_v}\|_F\leq (\DIFdelbegin \DIFdel{\gamma_{25}}\DIFdelend \DIFaddbegin \tilde{\gamma}\DIFadd{_{10}}\DIFaddend ^{(l)} + \tilde{\gamma}_{m}^{(h)}).\label{eqn:mpapplyP}
\end{equation}
...
\|\Delta \bb{y}\|_2 &\leq (\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)}+n\tilde{\gamma}_m^{(h)})\|\bb{x}\|_2,\;\; \|\Delta \bb{Q}\|_F\leq (\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)}+n\tilde{\gamma}_m^{(h)}).\label{eqn:mp19.3}
\end{align} 
\DIFdelbegin \DIFdel{Note that we use the $\tilde{\gamma}^{(l)}$ notation, where the small integer $c$ is now required to be $\cO(25)$.
}\DIFdelend %DIF > Note that we use the $\tilde{\gamma}^{(l)}$ notation, where the small integer $c$ is now required to be $\cO(14)$.
...
\begin{theorem}
	...
	Then,
	\begin{align}
	\|\Delta \bb{R}_{mpHQR2}[:,j]\|_2&\leq (\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)}+n\tilde{\gamma}_m^{(h)}) \|\bb{A}[:,j]\|_2,\;\; \|\Delta \bb{R}_{mpHQR2}\|_F\leq (\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)}+n\tilde{\gamma}_m^{(h)}) \|\bb{A}\|_F \label{eqn:mpHQR2R}\\
	\|\Delta \bb{Q}[:,j]_{mpHQR2}\|_2&\leq (\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)}+n\tilde{\gamma}_m^{(h)}),\;\; \|\Delta \bb{Q}_{mpHQR2}\|_F \leq n^{1/2} (\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)}+n\tilde{\gamma}_m^{(h)})\label{eqn:mpHQR2Q}.
	\end{align}
\end{theorem}
Unsurprisingly, the inner product mixed precision setting yields higher error bounds \DIFaddbegin \DIFadd{(see \mbox{%DIFAUXCMD
\cref{thm:feHQR}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend as it uses more low precision arithmetic than the settings described in \cref{sec:mp-3}. 
\DIFaddbegin \DIFadd{Before commenting on the significance of \mbox{%DIFAUXCMD
\cref{thm:mpHQR}}\hspace{0pt}%DIFAUXCMD
, we show that the same bounds hold for the BQR variant.
}\DIFaddend In the next sections we analyze using {\tt mpHQR2} instead of {\tt HQR} within \cref{algo:blockHQR,algo:par_tsqr}.
...
\begin{equation}
\|\hat{\bb{X}}_{k}^{(l)}- \bb{X}_k\|_F = \|(\hat{\bb{P}}_k^{(1)}\cdots \hat{\bb{P}}_k^{(r)})-(\bb{P}_k^{(1)}\cdots \bb{P}_k^{(r)}))\|_F \leq \tilde{\gamma}\DIFdelbegin \DIFdel{_{r}}\DIFdelend \DIFaddbegin \DIFadd{_{10r}}\DIFaddend ^{(l)} + r\tilde{\gamma}_{m}^{(h)}.
\end{equation}
...
\begin{align}
\|\hat{\bb{R}}_{mpBQR2}[:,j]\|_2 &= \|\hat{\bb{X}}_1\cdots\hat{\bb{X}}_N\bb{A}[:,j]\|_2\leq\left( N\tilde{\gamma}\DIFdelbegin \DIFdel{_{r}}\DIFdelend \DIFaddbegin \DIFadd{_{10r}}\DIFaddend ^{(l)} + n\tilde{\gamma}_{m}^{(h)}\right)\|\bb{A}[:,j]\|_2,\\
\|\hat{\bb{Q}}_{mpBQR2}\|_F &\leq n^{1/2}\left(N\tilde{\gamma}\DIFdelbegin \DIFdel{_{r}}\DIFdelend \DIFaddbegin \DIFadd{_{10r}}\DIFaddend ^{(l)} + n\tilde{\gamma}_{m}^{(h)}\right) \approx \left(1+\DIFdelbegin \DIFdel{\frac{M_{l,h}}{m}}\DIFdelend \DIFaddbegin \DIFadd{\frac{10M_{l,h}}{m}}\DIFaddend \right)n^{3/2}\tilde{\gamma}_{m}^{(h)}. \label{eqn:mpBQR2}
\end{align}
...
\DIFdelbegin \DIFdel{Furthermore, the $\tilde{\gamma}^{(l)}$ in this section requires $c=\cO(25)$, whereas the same notation in \mbox{%DIFAUXCMD
\cref{sec:mp-3b} }\hspace{0pt}%DIFAUXCMD
assumes $c$ to be a }\emph{\DIFdel{small}} %DIFAUXCMD
\DIFdel{positive integer.
Therefore, the }\DIFdelend %DIF > Furthermore, the $\tilde{\gamma}^{(l)}$ in this section requires $c=\cO(14)$, whereas the same notation in \cref{sec:mp-3b} assumes $c$ to be a \emph{small} positive integer.
%DIF > Therefore, the numerical stability of {\tt mpBQR2} is guaranteed at smaller matrix sizes than the numerical stability of {\tt mpBQR3} and BQR in high precision.
\DIFaddbegin \DIFadd{Therefore, guarantees of }\DIFaddend numerical stability of {\tt mpBQR2} \DIFdelbegin \DIFdel{is guaranteed at }\DIFdelend \DIFaddbegin \DIFadd{are limited to }\DIFaddend smaller matrix sizes \DIFdelbegin \DIFdel{than the numerical stability }\DIFdelend \DIFaddbegin \DIFadd{when compared to those }\DIFaddend of {\tt mpBQR3} and BQR in high precision.
...
This corresponds to replacing every instance of $n\tilde{\gamma}_{m'}$ for \DIFdelbegin \DIFdel{$m'\in\{2n, m2^{-L}\}$ }\DIFdelend \DIFaddbegin \DIFadd{$m'\in\{2n, 2^{-L}m\}$ }\DIFaddend in \cref{thm:moriTSQR} with \DIFdelbegin \DIFdel{$\tilde{\gamma}_n^{(l)} + n\tilde{\gamma}_{m'}^{(h)}$}\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\gamma}_{10n}^{(l)} + n\tilde{\gamma}_{m'}^{(h)}$}\DIFaddend .
...
\begin{equation}
\|\hat{\bb{Q}}_{mpTSQR2}[:,j] -\bb{Q}[:,j]\|_2 \leq (L+1)\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)} +n(\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)} + L\tilde{\gamma}_{ 2n}^{(h)}).\label{eqn:mptsqr2Qcol}
\end{equation} 
Then, the matrix norm error bound is 
\begin{align}
\|\hat{\bb{Q}}_{mpTSQR2}-\bb{Q}\|_F \leq n^{1/2}(L+1)\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)} +n^{3/2}(\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)} + L\tilde{\gamma}_{ 2n}^{(h)})\\
\approx \left(1+ \DIFdelbegin \DIFdel{\frac{M_{l,h}L}{m2^{-L}+ 2Ln}}\DIFdelend \DIFaddbegin \DIFadd{\frac{10M_{l,h}L}{2^{-L}m+ 2Ln}}\DIFaddend \right)n^{3/2}(\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)} + L\tilde{\gamma}_{ 2n}^{(h)}),\label{eqn:mptsqr2Q}
\end{align}
...
\begin{equation*}
1\gg n^{1/2}\left(\tilde{\gamma}\DIFdelbegin \DIFdel{_{n}}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)} + n\tilde{\gamma}_{m}^{(h)}\right) \gg n^{1/2}\left((L+1)\tilde{\gamma}\DIFdelbegin \DIFdel{_n}\DIFdelend \DIFaddbegin \DIFadd{_{10n}}\DIFaddend ^{(l)} +n(\tilde{\gamma}\DIFdelbegin \DIFdel{_{m2^{-L}}}\DIFdelend \DIFaddbegin \DIFadd{_{2^{-L}m}}\DIFaddend ^{(h)} + L\tilde{\gamma}_{ 2n}^{(h)})\right).%,
\end{equation*}
...
While large $L$ can imply \DIFdelbegin \DIFdel{$m\gg m2^{-L}+2Ln$, it does not always lead to $d \gg d_1+Ld_2$}\DIFdelend \DIFaddbegin \DIFadd{$m\gg 2^{-L}m+2Ln$, it is not always the case}\DIFaddend .
Although the theoretical error bounds do not give a clear indication of the worst-case performances of HQR and TSQR in \DIFdelbegin \DIFdel{mixed-precision }\DIFdelend \DIFaddbegin \DIFadd{mixed precision }\DIFaddend settings, TSQR outperformed HQR on ill-conditioned matrices within our numerical simulations.
%DIF > $d \gg d_1+Ld_2$.
These experiments are discussed in detail in \DIFdelbegin \DIFdel{the next section}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{sec:NE}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
%DIF > We increased the number of rows $m$ from $1000$ to $13949$, while keeping $n=m/4$, $r=n/4$, and $L=2$ and the test matrices were sampled from the standard normal distribution. 
We increased the number of rows $m$ from $1000$ to $13949$, while keeping \DIFdelbegin \DIFdel{$n=m/4$, $r=n/4$}\DIFdelend \DIFaddbegin \DIFadd{$n=250$, $r=63$}\DIFaddend , and $L=2$ and the test matrices were sampled from the standard normal distribution.
...
\begin{figure}[h!]
	\centering
	\vspace{-10pt}
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.45\textwidth]{./figures/sizefig.png}
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF > \includegraphics[width=0.45\textwidth]{./figures/sizefig.png}
	\DIFaddbeginFL \includegraphics[width=0.45\textwidth]{./fixedn.png}
	\DIFaddendFL \includegraphics[width=0.45\textwidth]{./figures/mpBQR3-blocksize-1108.png}
...
The test matrices were generated following example from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Blanchard2019} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Blanchard2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend by setting $\bb{A}={\tt castdown}(\bb{Q}_1\bb{D}\bb{Q}_2)$ where $\bb{Q}_1\in\F_h^{m\times n}$, $\bb{Q}_2\in\F_h^{n\times n}$ are orthogonal and $\bb{D}=\mathrm{Diagonal}(\{\log_{10}(0),\cdots, \log_{10}(-3)\})\in\F_h^{n\times n}$.  
...
Lastly, we compared \DIFaddbegin \DIFadd{a mixed precision variant of a communication-avoiding QR algorithm (}\DIFaddend {\tt mpTSQR2}\DIFdelbegin \DIFdel{against }\DIFdelend \DIFaddbegin \DIFadd{) against a mixed precision variant of the HH QR algorithm (}\DIFaddend {\tt mpHQR2}\DIFaddbegin \DIFadd{) on a set of fixed-size matrices with varying condition numbers}\DIFaddend .
...
\DIFaddbegin \DIFadd{Even though the condition number is not a part of our rounding error analysis, we use it in this experiment in an attempt to reach the ``worst-case'' scenario described by our deterministic error bounds in \mbox{%DIFAUXCMD
\cref{sec:mpanalysis}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend The relative backward error, $\|\hat{\bb{Q}}\hat{\bb{R}}-\bb{A}\|_F/\|\bb{A}\|_F$, was computed by casting up $\hat{\bb{Q}}$, $\hat{\bb{R}}$, and $\bb{A}$ to fp64 to compute the Frobenius norms.
...
These results suggests that TSQR can outperform HQR \DIFdelbegin \DIFdel{even }\DIFdelend in mixed precision settings, and particularly when HQR \DIFdelbegin \DIFdel{is unstable due to larger condition numbers}\DIFdelend \DIFaddbegin \DIFadd{suffers from accumulated rounding errors}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \vspace{-5pt}
...
Also, a special case is shown that demonstrate {\tt mpTSQR2} can outperform {\tt mpHQR2} \DIFaddbegin \DIFadd{in accuracy }\DIFaddend despite having higher error bounds.
...
We present the algorithm and standard error analysis of HQR and its blocked variants (BQR and TSQR), modify the algorithms to support two mixed precision settings, and performed error \DIFdelbegin \DIFdel{analysis that accurately }\DIFdelend \DIFaddbegin \DIFadd{analyses that }\DIFaddend bound the mixed precision versions.
...
Meanwhile, we should rely on the error bounds formed in \cref{sec:mpanalysis}\DIFaddbegin \DIFadd{.
}\section*{\DIFadd{Acknowledgements}}
\DIFadd{The authors are grateful to the anonymous reviewers for their constructive criticisms, which imporved the clarity of the presentation}\DIFaddend .
