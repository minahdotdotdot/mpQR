...
Finally, the error bounds for the $\bb{Q}$ matrix formed from implementing all of the algorithms discussed in this section are summarized in \cref{table:summary}.
\begin{table}[H]
	\center
 ...
 TSQR & $n^{3/2}(\tilde{\gamma}_{m2^{-L}}+L\tilde{\gamma}_{2n})$ &  \begin{tabular}{@{}c@{}}$n^{1/2}\tilde{\gamma}_{10(L+1)}^{(l)}$\\ $+n^{3/2}(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{m2^{-L}}^{(h)})$\end{tabular} & \begin{tabular}{@{}c@{}}$n^{1/2}\tilde{\gamma}_{L+1}^{(l)}$ \\ $+n^{3/2}\left(L\tilde{\gamma}_{2n}^{(h)}+\tilde{\gamma}_{2^{-L}m}^{(h)}\right)$\end{tabular}\\
 \hline
\end{tabular}
	\caption{Error bounds for $\|\Delta \bb{Q}\|_F$ for all algorithms in \cref{sec:HQR,sec:mpanalysis}.} 
	\label{table:summary},
\end{table} 
\vspace{-1cm}
