...
%DIF ADD revision2.tex   Fri Oct  9 20:44:04 2020
%Packages
...
}\DIFdelend \DIFaddbegin \DIFadd{storage.}\par
\DIFadd{Lastly, suppose that $\hat{y}$ is the result of implementing an algorithm that approximates $y=f(x)$.
...
Let $\bb{x}^{(l)},\bb{y}^{(l)}$ be $m$-length vectors stored in a low precision type ($\F_l^m$), $s_k$ be the exact $k^{th}$ partial sum, and \DIFdelbegin \DIFdel{$\hat{s_k}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{s}_k$ }\DIFaddend be $s_k$ computed with FLOPs.
