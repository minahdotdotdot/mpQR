...
#=
abstract type ptcc end
struct par_TSQR_components_a <: ptcc

struct par_TSQR_components_b <: ptcc

function initProblem{T<:AbstractFloat}(A::Array{T,2}, L::Int; hhvec::String="n1")
    m = size(A)[1]; n = size(A)[2];
    h = floor(Int, m/(2^L));           # height of small blocks
    ptc = Dict("n1"=> par_TSQR_components_a, "n2" => par_TSQR_components_b, "nn"=> par_TSQR_components_a)
    n_stop = Vcolumns(h, n)
    levels = Array{Array{ptc[hhvec],1},1}(L+1)
    # Initialize number of blocks at each level.
    for i = 1 : length(levels)
        levels[i] = Array{ptc[hhvec],1}(2^(L-(i-1)))
    end
    # Initialize the first level by writing A as a block-column. 
    for j = 1 : 2^L
        if j == 2^L # Last block may have more rows. 
            n_stop = Vcolumns(m - (2^L-1)*h, n)
            levels[1][j] = ptc[hhvec](A[(j-1)*h+1: end, :]);
        else
            levels[1][j] = ptc[hhvec](A[(j-1)*h+1: j*h, :]);
        end
    end
    return levels
end

function par_TSQR_compute{T<:AbstractFloat, TT<: ptcc}(levels::Array{Array{TT,1},1}, A::Array{T,2}; hhvec::String="n1")
    ptc = Dict("n1"=> par_TSQR_components_a, "n2" => par_TSQR_components_b, "nn"=> par_TSQR_components_a)
    L = length(levels)-1
    m = size(A)[1]; n = size(A)[2];
    if L == 1
    ...
    for i = 1 : L
        for j = 1 : 2^(L-i)
            if hhvec == "n2" || hhvec == "nn"
                ...
                levels[i][2*j].b[:],   levels[i][2*j].V[:, :],   levels[i][2*j].R[:, :]   = hh_QR(levels[i][2*j].A,   want_Q=false, hhvec=hhvec);
            elseif hhvec == "n1"
                levels[i][2*j-1].V[:, :], levels[i][2*j-1].R[:, :] = hh_QR(levels[i][2*j-1].A, want_Q=false, hhvec=hhvec);
                levels[i][2*j].V[:, :],   levels[i][2*j].R[:, :]   = hh_QR(levels[i][2*j].A,   want_Q=false, hhvec=hhvec);
            else
                error("hhvec needs to be n1, n2, or nn.\n")
            end
            levels[i+1][j] = ptc[hhvec]([levels[i][2*j-1].R; levels[i][2*j].R], n) # n_stop= n always since these A's are 2n by n. 
        end
    end
    levels[end][1] = ptc[hhvec]([levels[L][1].R; levels[L][2].R], n);
